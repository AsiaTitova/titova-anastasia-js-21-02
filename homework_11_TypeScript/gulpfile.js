...
gulp.task('copy-css-file', () => gulp.src(path.style) // Задача на копирование html файлов из src в dist
...
gulp.task('serve', () => {
  ...
  gulp.watch(['./src/*.html', './src/css/*.css']).on('change', gulp.series('copy-file'))
})

const whatchifyBrowserify = watchify(browserify( // Обёртка для слежения за изменениями ts
  {
    basedir: '.',
    entries: ["./src/ts/main.ts"], // Фалйы для обработки
    cache: {},
